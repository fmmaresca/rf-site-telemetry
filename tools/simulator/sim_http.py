...
class DeviceState:
    ...
    down_until: float = 0.0   # epoch seconds; if now < down_until -> skip sending
...
def main() -> None:
    ...
    ap.add_argument("--skip", default="", help="comma-separated device_ids to skip, e.g. rfsite-007,rfsite-010")
    ap.add_argument("--drop-rate", type=float, default=0.0,
                help="probability per send attempt to start an outage for a device (e.g. 0.002)")
    ap.add_argument("--drop-min-s", type=float, default=60.0, help="min outage duration seconds")
    ap.add_argument("--drop-max-s", type=float, default=240.0, help="max outage duration seconds")
    ...
    print(f"Simulator running: tenant={args.tenant} devices={len(states)} (requested {args.devices}) -> {base}/v1/ingest")
    ...
    while True:
        ...
        # If currently in outage, skip sending for this device
        if now < ds.down_until:
            time.sleep(0.01)
            continue

        # Randomly start an outage
        if args.drop_rate > 0 and random.random() < args.drop_rate:
            dur = random.uniform(args.drop_min_s, args.drop_max_s)
            ds.down_until = now + dur
            print(f"[DROP] {ds.device_id} outage for {dur:.0f}s")
            time.sleep(0.01)
            continue
